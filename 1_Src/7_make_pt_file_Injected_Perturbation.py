# 制作pt文件，用于模型训练
from utils import read_uc_from_stand_json,print_location,check_all_under_same_method,local_id_to_global_id,mix_items,unify_ncet_format
from imports import torch,REPLACEMENT_MAP,re,ARGC_20,PUB_GROUPING_UC_20_1, string,chain,Data,MODEL,Counter,GROUPING_UC_20,random,datetime,importlib
...
def create_edges_af(uc_list,uc_list_bp):
    if len(uc_list) != len(uc_list_bp):
        print(f'ERROR!! uc_list and uc_list_bp length not match!')
    ...
    for uc,uc_bp in zip(uc_list,uc_list_bp):
        ...
        if len(uc['Alt. Flow'])>0:
            edges_dict = module.make_bf_af_edges(uc['BF act'], uc['AF act'],edges_dict,bp_list,'BF act','AF act')
            edges_dict = module.make_bf_af_edges(uc['BF obj'], uc['AF obj'],edges_dict,bp_list,'BF obj','AF obj')
...
def generate_dataset_4turbo(edges_dict, node_to_UCText, para_dict):
    ...
    for key_node in node_to_id:
        if 'keyword' in key_node or 'BF' in key_node: # 20250915前:if 'keyword' in key_node:
            ...
            if len(related_node) == 0:  # line517-518,20250915 加  # 专门针对没有关联节点的keyword/bfnode，补充-1，不然会被认为是错误节点
...
def get_edges_dict_and_node_to_UCText_chatgpt(use_case_list, percentage_num,uc_list_bp):
    edges_dict_list = []
    node_to_UCText_list = []
    for uc_list_index, uc_list in enumerate(use_case_list):
        if uc_list_index >= 30:  # ncet数据集当进行到338之后，eval和test数据集在评估鲁棒性的时候需要人工加入对bf node的扰动。pub数据集是30
            uc_list = bf_node_number_percentage(uc_list, percentage_num)

        edge_dict, node_to_UCText = create_edges_af(uc_list,uc_list_bp[uc_list_index])
        edges_dict_list.append(edge_dict)
        node_to_UCText_list.append(node_to_UCText)

    return edges_dict_list, node_to_UCText_list

# 评估鲁棒性的时候需要人工扰动bf node
def bf_node_number_percentage(uc_list, percentage_num):
    for uc in uc_list:
        uc['BF act'] = keep_sequece(uc['BF act'],percentage_num)
        uc['BF obj'] = keep_sequece(uc['BF obj'], percentage_num)
    return uc_list

def keep_sequece(lst_old,percentage_num):
    # 拉平前记录每个子列表长度
    lengths = [len(sublist) for sublist in lst_old]

    # 对flat_list做变换...
    lst = replace_random_n_percent(flatten_list(lst_old), percentage_num)

    # 按原长度切分回二级列表
    new_bf_act = []
    idx = 0
    for l in lengths:
        new_bf_act.append(lst[idx:idx+l])
        idx += l
    lst = new_bf_act
    return lst


# 按照百分比，获得列表中的item，并按照原始顺序输出
def get_percent_elements(input_list, percent_num):
    # 计算需要选择的元素数量，约为80%
    num_elements_to_select = int(round(len(input_list) * percent_num))
    ...
    # 根据选择的索引提取元素，并保持顺序
    selected_elements = [input_list[i] for i in sorted(selected_indices)]

    return selected_elements


def replace_random_n_percent(lst,percent_num):
    lst_old = lst.copy()  # debug时记录用的
    if not lst:  # 空列表直接返回
        return lst
    
    n = max(1, int(len(lst) * (1-percent_num)))  # 至少选1项
    indices = random.sample(range(len(lst)), n)  # 随机选不重复索引
    
    for idx in indices:
        original = lst[idx]
        candidates = [x for x in lst if x != original]  # 筛选不同值的候选池
        if not candidates:  # 若全列表相同，无法替换
            continue
        new_val = random.choice(candidates)  # 从候选池选新值
        lst[idx] = new_val
    return lst
...
if __name__ == '__main__':
    task_name = 'make_pt_perturbation_pub'
    print(f"*** task_name: {task_name}, Starting time: {datetime.now()} !!!!! ***")

    if task_name =="make_pt_perturbation_pub": # 模仿BFGen的代码：_chatgpt_ncet.py/task0
        ...
        for i in [90,80,70,60,50]:
            percentage_num = i/100
            pt_save_path = "E:/Trae_project/AFGen_pt_file/3_with_branching_point/2_perturbation/" + str(i) + "_gpt_pub_llm.pt"

            for in_file_path, pt_save_path in zip([in_file_path_gpt],[pt_save_path]):
                ...
                edges_dict_list, node_to_UCText_list = get_edges_dict_and_node_to_UCText_chatgpt(uc_list, percentage_num,uc_list_bp)

                # 5.1 统计：所有子图中最多的节点数（act+obj+key）
                argc2 = count_node_data_dict_sub(edges_dict_list)
                # 5.2 判断两次统计是否一致. 
                print(f'全局统计和分图统计不一致的项为：{find_diff_dict(argc1, argc2)}') # 这里node_total因为node_bf_act和node_bf_obj加入的随机错误node（扰动），node数目会发生变化。
    ...
    if task_name =="make_pt_perturbation_ncet": # 模仿 7_ncet.py/make_pt_ncet 的代码
        ...
        for i in [90,80,70,60,50]:
            percentage_num = i/100
            pt_save_path = "E:/Trae_project/AFGen_pt_file/2_perturbation/" + str(i) + "_gpt_ncet_llm.pt"

            for in_file_path, pt_save_path, bf_path in zip([af_node_ground_truth_gpt],[pt_save_path],[bf_node_gpt]):
                # 检测文件是同一个method的
                ...
                # 3. 生成全局edge数目和 node 数目
                argc1 = check_agc_cn(uc_list)

                # 4. 进行分组（使用固定分组/随机分组）
                uc_list = group_uc_fixed(uc_list, GROUPING_UC_20)

                # 5. 获得两个字典：edges_dict(存放act/obj/keyword之间的edge)；node_to_UCText_list(存放act/obj/keyword to uctext的映射)
                edges_dict_list, node_to_UCText_list = get_edges_dict_and_node_to_UCText_chatgpt(uc_list, percentage_num)

                # 5.1 统计：所有子图中最多的节点数（act+obj+key）
                argc2 = count_node_data_dict_sub(edges_dict_list)
                # 5.2 判断两次统计是否一致. 
                print(f'全局统计和分图统计不一致的项为：{find_diff_dict(argc1, argc2)}') # node_bf_act和node_bf_obj加入的随机错误node（扰动），node数目会发生变化。
